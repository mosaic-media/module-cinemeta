...
// (architecture#3), so a dependency added here is a dependency the Platform and
// every other core module must resolve at a compatible version. The boundary is
// what keeps the tier a delivery decision rather than a contract one: this
// module is written exactly as a third party's would be, and could move out of
// process as a build change rather than a rewrite (platform#39).
//
// Note the absence of an sdui exemption. A module gets one to contribute its own
// settings screen (sdk#4); this module has no settings, so it has no reason to
// reach the UI contract at all.
